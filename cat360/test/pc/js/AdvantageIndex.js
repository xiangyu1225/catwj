...
function advantage_logonstatus() {
    ...
    if (p != "" && p != null) {
        $("#hidphone").val(p);
        var data2 = "phone=" + p;
        $.post("http://cattest.hnqxs.com/Interface/Login?sj=" + Math.random(), data2, function (dat2) {
            $(".DLink.DLink_1").hide();
            $(".DLink.DLink_2").hide();
            $(".DLink.DLink_3").show();
            $(".DLink.DLink_4").show();
        ...
        //判断用户登录状态
        var data = "";
        $.post("http://cattest.hnqxs.com/Interface/getLoginStatus?sj=" + Math.random(), data, function (dat) {
...
var data = "name=" + escape(name) + "&phone=" + phone + "&source=" + source + "&utmsource=" + src + "&utmmedium=" + mdm + "&utmcampaign=" + cpn + "&utmcontent=" + ctt + "&utmterm=" + escape(tem)+"&ExcavatorModel=&ExcavatorSize=";

    // $.post("http://cattest.hnqxs.com/Interface/Add_360Advantage?sj=" + Math.random(), data, function (dat) {
    $.post("http://cattest.hnqxs.com/Interface/Add_360Advantage?sj=" + Math.random(), data, function (dat) {
        ...
        if (dat != null && dat != '') {
            ...
            switch (dat.ErrorCode) {
                case 0:
                    alert('注册成功<br>欢迎您继续访问');
                    $('.pageFormPop .btn').show();
                    FormLineChange();
                    $("#hidphone").val(phone);
...
function AdvantageMobileReg() {
    ...
    $.post("http://cattest.hnqxs.com/Interface/Regist?sj=" + Math.random(), data, function (dat) {
        if (dat != null && dat != '') {
            if (dat.success == '1') {
...
function AdvantageCheckPhoneExits(str) {
    ...
    $.post("http://cattest.hnqxs.com/Interface/phoneExits?sj=" + Math.random(), { phone: str }, function (txt) {
...
function AdvantageMobileLogin() {
    ...
    if (phoneNo != null && phoneNo != '') {
        $.post("http://cattest.hnqxs.com/Interface/Login?sj=" + Math.random(), data, function (dat) {
            if (dat != '') {
                if (dat.success == '1') {
